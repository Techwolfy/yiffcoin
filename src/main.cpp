...
uint256 hashGenesisBlock("0x6845ecca7135003d222ab4f2b2da47445c508f2058a92ed4a81094aa794a0889");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yiffcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yiffcoin Signed Message:\n";
...
{
    // Yiffcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 6.9 * 24 * 60 * 60; // Yiffcoin: 6.9 days
static const int64 nTargetSpacing = 69; // Yiffcoin: 69 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yiffcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("yiffcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yiffcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Yiffcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6845ecca7135003d222ab4f2b2da47445c508f2058a92ed4a81094aa794a0889");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Yiffcoin created on 14 March 2014.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394755200;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1976421;
        ...
        assert(block.hashMerkleRoot == uint256("0x465e865a3e57c28e3e749a00789fee1192f5fc3c66f3f37bcb1dff30b771d685"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xff, 0x69, 0x69 }; // Yiffcoin: unique network magic value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YiffcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YiffcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YiffcoinMiner(CWallet *pwallet)
{
    printf("YiffcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yiffcoin-miner");
    ...
    try { loop {
        ...
        printf("Running YiffcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YiffcoinMiner terminated\n");
        throw;
    }
}

void GenerateYiffcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YiffcoinMiner, pwallet));
}
