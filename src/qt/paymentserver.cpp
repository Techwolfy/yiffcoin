...
const int YIFFCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString YIFFCOIN_IPC_PREFIX("yiffcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(YIFFCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(YIFFCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(YIFFCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click yiffcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start yiffcoin: click-to-pay handler");
    else
...
{
    // clicking on yiffcoin: URLs creates FileOpen events on the Mac:
