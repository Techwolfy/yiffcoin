...
{
    QString ret = QString("yiffcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non YIFF unit input in lnReqAmount, we generate the URI with YIFF as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(YiffcoinUnits::format(YiffcoinUnits::YIFF, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
